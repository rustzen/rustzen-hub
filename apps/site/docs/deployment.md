# apps/site Deployment

## Current Deployment Class

`apps/site` is a Vercel static product site deployed for `rustzen.dev`. Deployment is governed by `vercel.json` and the Astro build output, not by Rust service deployment conventions.
...
| custom domains | `rustzen.dev` verified on Vercel during the rename cleanup |
